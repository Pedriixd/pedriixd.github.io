document.addEventListener('DOMContentLoaded', function() {
    ...
    function saveImagesToStorage() {
        // Limitar el número de imágenes guardadas para no exceder el límite de localStorage
        const maxImages = 20;
        if (uploadedImages.length > maxImages) {
            uploadedImages = uploadedImages.slice(-maxImages);
        }
        ...
        try {
            ...
            // Si hay un error (por ejemplo, cuota excedida), intentar guardar menos imágenes
            if (uploadedImages.length > 5) {
                uploadedImages = uploadedImages.slice(-5);
                try {
                    localStorage.setItem('pedriixd_images', JSON.stringify(uploadedImages));
                } catch (e) {
                    console.error('Error al guardar imágenes reducidas:', e);
                }
            }
        }
    }
    
    // Función para cargar imágenes guardadas del almacenamiento local
    function loadSavedImages() {
        try {
            const savedImages = localStorage.getItem('pedriixd_images');
            if (savedImages) {
                uploadedImages = JSON.parse(savedImages);
                // Agregar las imágenes guardadas a la galería
