document.addEventListener('DOMContentLoaded', function() {
    ...
    // Cargar imágenes guardadas del localStorage al iniciar
    loadSavedImages();
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (file.type.match('image.*')) {
                    ...
                    reader.onload = function(e) {
                        ...
                        // Agregar la imagen a la lista de imágenes subidas
                    ...
                    // Leer la imagen como URL de datos (mantiene la imagen completa)
    ...
    // Función para guardar imágenes en el almacenamiento local
    function saveImagesToStorage() {
        try {
            // Usamos IndexedDB para guardar imágenes permanentemente
            const request = indexedDB.open('PedriixdImagesDB', 1);
            ...
            request.onsuccess = function(e) {
                ...
                // Almacenar solo las nuevas imágenes
                const lastImage = uploadedImages[uploadedImages.length - 1];
                store.add({ imageData: lastImage, date: new Date().toISOString() });
                ...
                transaction.onerror = function(event) {
                    console.error('Error al guardar imagen en IndexedDB:', event.target.error);
                };
            };
            ...
            request.onerror = function(event) {
                ...
                try {
                    localStorage.setItem('pedriixd_images', JSON.stringify(uploadedImages));
                } catch (e) {
                    console.error('Error al guardar imágenes en localStorage:', e);
                }
            };
        } catch (e) {
            console.error('Error general al guardar imágenes:', e);
        }
    }
    
    // Función para cargar imágenes guardadas del almacenamiento
                ...
                if (db.objectStoreNames.contains('images')) {
                    ...
                    getAllRequest.onsuccess = function(e) {
                        ...
                        if (images && images.length > 0) {
                            // Recuperar las imágenes y añadirlas a la galería
                            images.forEach(item => {
                                uploadedImages.push(item.imageData);
                                addImageToGallery(item.imageData);
                            });
                        } else {
                            // Si no hay imágenes en IndexedDB, intentar cargar desde localStorage como respaldo
    ...
    function fallbackToLocalStorage() {
        try {
            const savedImages = localStorage.getItem('pedriixd_images');
            if (savedImages) {
            ...
            console.error('Error al cargar imágenes del localStorage:', e);
        }
    }
    
    // Botón de compartir por WhatsApp
    shareButton.addEventListener('click', function() {
        // Validación: asegurarse de que hay stickers en el canvas
        if (stickersOnCanvas.length === 0) {
            alert('Agrega algunos stickers antes de enviar.');
            return;
        }
        
        // Mensaje para WhatsApp
        const message = encodeURIComponent("¡Hola! Aquí está mi plancha de stickers personalizada creada con Pedriixd. Quisiera solicitar este diseño.");
    ...
    function addImageToGallery(imageUrl) {
        ...
        img.src = imageUrl;
        img.onload = function() {
            // Asegurar que la imagen cargue completamente
    ...
    function handleDragStart(e) {
        ...
        if (img) {
            ...
            // Crear una imagen fantasma para el arrastre que mantiene la proporción
    ...
    function addStickerToCanvas(imageUrl, position = null) {
        ...
        // Añadir la imagen al sticker manteniendo la imagen completa
        const img = document.createElement('img');
        img.src = imageUrl;
        img.style.width = '100%';
        img.style.height = '100%';
        img.style.objectFit = 'contain'; // Mostrar la imagen completa
